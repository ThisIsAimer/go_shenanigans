...
func (c *calculateClient) GenerateFibonacchi(ctx context.Context, in *FibonacchiRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[FibonacchiResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Calculate_ServiceDesc.Streams[0], Calculate_GenerateFibonacchi_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[FibonacchiRequest, FibonacchiResponse]{ClientStream: stream}
...
func _Calculate_GenerateFibonacchi_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(FibonacchiRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(CalculateServer).GenerateFibonacchi(m, &grpc.GenericServerStream[FibonacchiRequest, FibonacchiResponse]{ServerStream: stream})
}
